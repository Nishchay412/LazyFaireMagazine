...
import { finance } from "/src/components/teaminfo.jsx";

export function Finance() {
  const reorderedExecutives = [
    ...finance.filter((item) => item.name === "Mohit Makhija"),
    ...finance.filter((item) => item.name !== "Mohit Makhija"),
  ];
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold mb-8 text-white">Our Finance Team</h1>
        ...
        {reorderedExecutives.map((item, index) => (
          <div
            ...
            className={`flex flex-col items-center p-4 bg-transparent rounded-lg shadow-md ${
              item.name === "Mohit Makhija" ? "border-2 border-blue-800" : ""
            }`}
          >
            <img
